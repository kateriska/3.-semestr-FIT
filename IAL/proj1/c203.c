...
*/
		...
		q->f_index = 0; // nastaveni zacatku fronty
		q->b_index = 0; // nastaveni prvniho volneho mista fronty

		for (int i = 0; i < QUEUE_SIZE; i++) // nastaveni vsech hodnot v poli
		{
			q->arr[i] = '*';
		}
...
*/
		...
		*c = q->arr[q->f_index]; // parametr c ukazuje na prvni index fronty
...
*/
		...
		q->f_index = nextIndex(q->f_index); // posunuti na dalsi index - odstrani se znak q ze zacatku fronty
